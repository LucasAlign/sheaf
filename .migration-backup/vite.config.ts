...
export default defineConfig(({mode})=>{
 ...
 for(const key of ['DATABASE_URL','SUPABASE_URL','SUPABASE_SERVICE_ROLE_KEY','SHEAF_ORG_ID','APP_URL','RESEND_API_KEY','EMAIL_FROM','CRON_SECRET','LINK_SIGNING_SECRET'])if(!process.env[key]&&env[key])process.env[key]=env[key];
