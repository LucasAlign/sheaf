...
4. `supabase/seed.sql`, after replacing the placeholder caseworker contact email.
...
The cron refreshes matches, plans due waves, queues reminders for claims older than 48 hours, reserves due messages, checks current preferences and claim state, then sends through Resend. Match/digest/reminder email caps are enforced at queue and delivery time. Reminders are deduplicated by need/day. Digest delivery is at 9 a.m. Eastern the next day, independent of UTC daylight-saving shifts.
...
6. Mark it complete. If a real eligible contribution was received by the organization, record that contribution before downloading the acknowledgment.
7. Verify unsubscribe preferences, link expiration, and the organization contact email before introducing real family needs.
